...
def home():
   ...
   # Set values
   waterLevel = "High" if float(status["WaterLevel"])==0 else "Low"
   waterTemperature = status["WaterTemp"]
   airTemperature = status["AirTemp"]
   phLevel = status["pH"]
   heaterStatus = "On" if float(status["Heater"])==1 else "Off"
   filterStatus = "On" if float(status["Filter"])==1 else "Off"
   if float(status["Pump"])==1:
